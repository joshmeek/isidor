...
def test_update_protocol():
    ...
    # Update data
    update_data = {
        "name": f"{TEST_PROTOCOL['name']} - Updated",
        "description": f"{TEST_PROTOCOL['description']} - Updated"
    }
...
def test_get_protocol_templates():
    ...
    response = requests.get(
        f"{BASE_URL}/protocols/templates/list",
        headers={"Authorization": f"Bearer {ACCESS_TOKEN}"}
    )
...
def test_create_from_template():
    ...
    response = requests.post(
        f"{BASE_URL}/protocols/templates/create",
        json=template_data,
        headers={"Authorization": f"Bearer {ACCESS_TOKEN}"}
    )
...
if __name__ == "__main__":
    try:
        ...
        if all([create_success, get_success, update_success, templates_success, 
               template_create_success, delete_success]):
