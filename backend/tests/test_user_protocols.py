...
def test_get_active_protocols():
    ...
    response = requests.get(
        f"{BASE_URL}/user-protocols/active",
        headers={"Authorization": f"Bearer {ACCESS_TOKEN}"}
    )
...
def test_get_user_protocol():
    ...
    # Get user protocol
    response = requests.get(
        f"{BASE_URL}/user-protocols/{TEST_USER_PROTOCOL_ID}",
        headers={"Authorization": f"Bearer {ACCESS_TOKEN}"}
    )
...
def test_get_protocol_progress():
    ...
    response = requests.get(
        f"{BASE_URL}/user-protocols/{TEST_USER_PROTOCOL_ID}/progress",
        headers={"Authorization": f"Bearer {ACCESS_TOKEN}"}
    )
...
def cleanup():
    ...
    if TEST_PROTOCOL_ID and ACCESS_TOKEN:
        ...
        response = requests.delete(
            f"{BASE_URL}/protocols/{TEST_PROTOCOL_ID}",
            headers={"Authorization": f"Bearer {ACCESS_TOKEN}"}
        )
...
if __name__ == "__main__":
    try:
        ...
        if all([enroll_success, get_all_success, get_active_success, get_one_success, 
               status_update_success, progress_success, complete_success, delete_success]):
