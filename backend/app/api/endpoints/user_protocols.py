...
from app.schemas.user_protocol import (
    UserProtocol as UserProtocolSchema,
...
def read_user_protocols(
    *,
    ...
    """
    user_protocols = get_user_protocols(
        db=db, 
...
def read_active_user_protocols(
    *,
    db: Session = Depends(get_db),
    current_user: UserSchema = Depends(get_current_active_user)
) -> Any:
...
def enroll_in_protocol(
    *,
    db: Session = Depends(get_db),
    protocol_in: UserProtocolCreate,
    current_user: UserSchema = Depends(get_current_active_user)
) -> Any:
    """
    ...
    user_protocol = enroll_user_in_protocol(
        db=db, 
        user_id=current_user.id, 
        protocol_id=protocol_in.protocol_id,
        start_date=protocol_in.start_date
    )
...
def update_status(
    *,
    ...
    updated_protocol = update_user_protocol_status(
        db=db, 
