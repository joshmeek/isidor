...
def create_health_metric(db: Session, obj_in: HealthMetricCreate) -> HealthMetric:
    ...
    embedding = generate_health_metric_embedding(
...
def find_similar_health_metrics(
    ...
    query = query.filter(
        HealthMetric.embedding.cosine_distance(query_embedding) <= max_distance
    ).order_by(
        HealthMetric.embedding.cosine_distance(query_embedding)
    ).limit(limit)
