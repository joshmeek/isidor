...
def generate_query_hash(query: str, metric_types: Optional[List[str]] = None, **kwargs) -> str:
    ...
    params = {"query": query, "metric_types": sorted(metric_types) if metric_types else None, **kwargs}
...
def get_cached_response(db: Session, user_id: UUID, endpoint: str, time_frame: str, query_hash: str) -> Optional[AICachedResponse]:
    """
...
def invalidate_cached_responses(db: Session, user_id: UUID, endpoint: Optional[str] = None, time_frame: Optional[str] = None) -> int:
    """
