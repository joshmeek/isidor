...
def create_or_update_ai_memory(db: Session, user_id: UUID, summary: str) -> AIMemory:
    ...
    if db_obj:
        ...
        # Create new memory
        db_obj = AIMemory(user_id=user_id, summary=summary, embedding=embedding)
        db.add(db_obj)
...
def find_similar_memories(db: Session, query_embedding: List[float], limit: int = 10, min_similarity: float = 0.7) -> List[AIMemory]:
    ...
    query = (
        db.query(AIMemory)
        .filter(AIMemory.embedding.cosine_distance(query_embedding) <= max_distance)
