...
def upgrade():
    ...
    op.create_index("ix_ai_cached_responses_lookup", "ai_cached_responses", ["user_id", "endpoint", "time_frame", "query_hash"])

    # Create index on expires_at for faster cleanup of expired entries
    op.create_index("ix_ai_cached_responses_expires_at", "ai_cached_responses", ["expires_at"])
